...
1. Spin up your jenkins-ansible server and navigate to Jenkins URL

Remenber to clone down your ansible-config-mgt repo if its a new jenkins-ansible server
...
pipeline {
  ...
  stages {
      stage("Initial cleanup") {
       ...
       stage('Checkout SCM') {
         steps{
            git branch: 'feature/jenkinspipeline-stages', url: 'https://github.com/onyeka-hub/ansible-config-proj-14.git'
         }
       }
