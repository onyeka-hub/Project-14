...
pipeline {
  ...
  stages {
      stage("Initial cleanup") {
       ...
       stage('Checkout SCM') {
         steps{
           git branch: 'feature', credentialsId: 'private-key', url: 'https://github.com/onyeka-hub/practice.git'
         }
       }
...
![summary of the ansible piaybook execution](./images/playbook-success-summary.PNG)
...
Notice that the default value loads up, but we can now specify which environment we want to deploy the configuration to. Simply type **sit.yml** or **ci.yml** and hit **build**
...
4. In Jenkins UI configure Artifactory. Go to **manage jenkins**, **configure system**. Then Configure the server ID, URL and Credentials, and run Test Connection. 
  - username: admin
...
5. Using Blue Ocean, create a multibranch Jenkins pipeline and point the pipeline to the php-todo repo forked.
...
![plot code covering report](./images/plot-code.PNG)
...
pipeline {
  ...
  stages {
    ...
    stage ('Deploy to Dev Environment') {
      steps {
        build job: 'ansible-config-proj-14/main', parameters: [[$class: 'StringParameterValue', name: 'env', value: 'dev']], propagate: false, wait: true
      }
    }
  }
}
